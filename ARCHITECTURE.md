...
| `Random` | `?@@`, `@`, `@` (3×3) | the only nondeterministic example: a `?` at the origin with an `@` at each of its four toroidal neighbours, verified against `stepRel`/`runRel` to halt in two steps whichever direction the `?` chooses, and (conversely) that *every* two-step relational run halts |
