...
example :
    ((run 5 { State.init (playfieldOf layoutProgram) with
        stack := [Int.ofNat 0, Int.ofNat 8], pc := (8, 6) }).map (fun s => (s.stack, s.pc)))
      = some ([Int.ofNat 4], (9, 10)) := by
  let S : State (playfieldWidth layoutProgram) (playfieldHeight layoutProgram) :=
    { State.init (playfieldOf layoutProgram) with
      stack := [Int.ofNat 0, Int.ofNat 8], pc := (8, 6) }
  change ((run 5 S).map (fun s => (s.stack, s.pc))) = some ([Int.ofNat 4], (9, 10))
...
example :
    ((run 1 { State.init (playfieldOf layoutProgram) with
        stack := [Int.ofNat 1, Int.ofNat 3], pc := (8, 6) }).map (fun s => (s.pc, s.dir)))
      = some ((8, 5), .up) := by
  let S : State (playfieldWidth layoutProgram) (playfieldHeight layoutProgram) :=
    { State.init (playfieldOf layoutProgram) with
      stack := [Int.ofNat 1, Int.ofNat 3], pc := (8, 6) }
  change ((run 1 S).map (fun s => (s.pc, s.dir))) = some ((8, 5), .up)
