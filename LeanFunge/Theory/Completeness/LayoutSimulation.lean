...
theorem encode_incCounter (c : Fin 2) (s : CMState) :
  ...
  fin_cases c
  · simpa [encodeState, incCounter, write, CMInstr.read, counterVal] using (encode_inc_c1_int s.c1 s.c2).symm
  · simpa [encodeState, incCounter, write, CMInstr.read, counterVal] using (encode_inc_c2_int s.c1 s.c2).symm
...
theorem encode_deczCounter (c : Fin 2) (s : CMState) (h : CMInstr.read c s ≠ 0) :
  ...
  fin_cases c
  · have h1 : 0 < s.c1 := by
      have : s.c1 ≠ 0 := by simpa [CMInstr.read] using h
      omega
    simpa [encodeState, decCounter, write, CMInstr.read, counterVal, h1] using (encode_decz_c1_int s.c1 s.c2 h1).symm
  · have h2 : 0 < s.c2 := by
      have : s.c2 ≠ 0 := by simpa [CMInstr.read] using h
      omega
    simpa [encodeState, decCounter, write, CMInstr.read, counterVal, h2] using (encode_decz_c2_int s.c1 s.c2 h2).symm
...
theorem encode_mod_zero (c : Fin 2) (s : CMState) :
  ...
  fin_cases c
  · simp [encodeState, counterVal, CMInstr.read]
    have hdvd : (2 : Int) ∣ (encode s.c1 s.c2 : Int) ↔ (2 : ℕ) ∣ encode s.c1 s.c2 := by
      simpa using (Int.natCast_dvd_natCast (m := 2) (n := encode s.c1 s.c2))
    rw [hdvd]
  ...
  · simp [encodeState, counterVal, CMInstr.read]
    have hdvd : (3 : Int) ∣ (encode s.c1 s.c2 : Int) ↔ (3 : ℕ) ∣ encode s.c1 s.c2 := by
      simpa using (Int.natCast_dvd_natCast (m := 3) (n := encode s.c1 s.c2))
    rw [hdvd]
