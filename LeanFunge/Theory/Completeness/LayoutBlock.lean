...
An `inc` block of the generated playfield executes its instruction: the body cells read back exactly
the block's snippet, and the instruction pointer steps through them. Running from a block's entry
multiplies the stack top by the counter digit and exits down the fall-through column, for arbitrary
programs.

The `decz` test cells `> : digit %` of a block on the generated playfield leave the encoded pair on
the stack with the remainder on top at the branch cell, for arbitrary programs.

A `jump` block of the generated playfield sends the pointer up its corridor column, and a `halt`
block stops the machine, for arbitrary programs.

The `|` branch cell of a `decz` block sends the pointer down through the decrement cells `digit / >
v` on an even encoding (counter positive), dividing the value and exiting down the fall-through
column, or up the corridor column on an odd encoding (counter zero), with the value unchanged.
...
* `deczBlock_decrement`: On an even encoding the block divides the value and exits down the
  fall-through column.

* `deczBlock_jump`: On an odd encoding the block sends the pointer up the corridor column with the
  value unchanged.
...
theorem incBlock_run (prog : CMProgram) (i : ℕ) (hi1 : i + 1 < prog.length)
  ...
  have hc0 : s.grid.get (entryColumn prog i) (blockRow prog i) = '>' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 0 0 (by exact Nat.zero_le _)
      (by exact blockHeight_pos _)
    rw [hinc] at hpb
    simpa only [blockBodyAt] using hpb
  have hc1 : s.grid.get (entryColumn prog i + 1) (blockRow prog i) = counterDigit c := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 1 0 (by rw [hinc]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hinc] at hpb
    simpa only [blockBodyAt] using hpb
  have hc2 : s.grid.get (entryColumn prog i + 2) (blockRow prog i) = '*' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 2 0 (by rw [hinc]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hinc] at hpb
    simpa only [blockBodyAt] using hpb
  have hc3 : s.grid.get (entryColumn prog i + 3) (blockRow prog i) = 'v' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 3 0 (by rw [hinc]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hinc] at hpb
    simpa only [blockBodyAt] using hpb
  have h1 : run 1 s = some { s with
    dir := .right,
    pc := (entryColumn prog i + 1, blockRow prog i) } := by
    ...
    rw [stepPos_right (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i)
      (blockRow prog i) (by omega) (by omega)]
  have h2 : run 1 { s with dir := .right, pc := (entryColumn prog i + 1, blockRow prog i) } =
some { s with dir := .right, stack := [counterVal c, n], pc := (entryColumn prog i + 2, blockRow
prog i) } := by
    rw [show run 1 { s with
      dir := .right,
      pc := (entryColumn prog i + 1, blockRow prog i) } = step { s with
        dir := .right,
        pc := (entryColumn prog i + 1, blockRow prog i) } by rfl]
    unfold step
    have hdec : decodeChar (counterDigit c) = .push (counterVal c) := by
      fin_cases c <;> rfl
    simp only [hsm, hc1, hdec, stepState]
    rw [stepPos_right (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 1)
      (blockRow prog i) (by omega) (by omega)]
    simp only [hstack, Stack.push]
  have h3 : run 1 { s with
    dir := .right,
    stack := [counterVal c, n],
    pc := (entryColumn prog i + 2, blockRow prog i) } =
some { s with dir := .right, stack := [n * counterVal c], pc := (entryColumn prog i + 3, blockRow
prog i) } := by
    rw [show run 1 { s with
      dir := .right,
      stack := [counterVal c, n],
      pc := (entryColumn prog i + 2, blockRow prog i) } = step { s with
        dir := .right,
        stack := [counterVal c, n],
        pc := (entryColumn prog i + 2, blockRow prog i) } by rfl]
    unfold step
    have hdec : decodeChar '*' = .mul := by unfold decodeChar; rfl
    simp only [hsm, hc2, hdec, stepState, Stack.applyBinary]
    rw [stepPos_right (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 2)
      (blockRow prog i) (by omega) (by omega)]
  have h4 : run 1 { s with
    dir := .right,
    stack := [n * counterVal c],
    pc := (entryColumn prog i + 3, blockRow prog i) } =
some { s with stack := [n * counterVal c], pc := (entryColumn prog i + 3, blockRow prog i + 1), dir
:= .down } := by
    rw [show run 1 { s with
      dir := .right,
      stack := [n * counterVal c],
      pc := (entryColumn prog i + 3, blockRow prog i) } = step { s with
        dir := .right,
        stack := [n * counterVal c],
        pc := (entryColumn prog i + 3, blockRow prog i) } by rfl]
    unfold step
    have hdec : decodeChar 'v' = .down := by unfold decodeChar; rfl
    simp only [hsm, hc3, hdec, stepState]
    rw [stepPos_down (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 3)
      (blockRow prog i) (by omega) (by omega)]
  have h12 : run (1 + 1) s = some { s with
    dir := .right,
    stack := [counterVal c, n],
    pc := (entryColumn prog i + 2, blockRow prog i) } := by
    exact run_append s { s with dir := .right, pc := (entryColumn prog i + 1, blockRow prog i) }
      (some { s with
        dir := .right,
        stack := [counterVal c, n],
        pc := (entryColumn prog i + 2, blockRow prog i) }) 1 1 h1 h2
  have h123 : run (1 + 1 + 1) s = some { s with
    dir := .right,
    stack := [n * counterVal c],
    pc := (entryColumn prog i + 3, blockRow prog i) } := by
    exact run_append s { s with
      ...
      (some { s with
        dir := .right,
        stack := [n * counterVal c],
        pc := (entryColumn prog i + 3, blockRow prog i) }) (1 + 1) 1 h12 h3
...
:= .down } := by
    exact run_append s { s with
      ...
      (some { s with
        stack := [n * counterVal c],
        pc := (entryColumn prog i + 3, blockRow prog i + 1),
        dir := .down }) (1 + 1 + 1) 1 h123 h4
...
theorem deczBlock_prefix (prog : CMProgram) (i : ℕ) (hi1 : i + 1 < prog.length)
  ...
  have hc1 : s.grid.get (entryColumn prog i + 1) (blockRow prog i) = ':' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 1 0 (by rw [hdecz]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hdecz] at hpb
    simpa only [blockBodyAt] using hpb
  have hc2 : s.grid.get (entryColumn prog i + 2) (blockRow prog i) = counterDigit c := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 2 0 (by rw [hdecz]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hdecz] at hpb
    simpa only [blockBodyAt] using hpb
  have hc3 : s.grid.get (entryColumn prog i + 3) (blockRow prog i) = '%' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 3 0 (by rw [hdecz]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hdecz] at hpb
    simpa only [blockBodyAt] using hpb
  have h1 : run 1 s = some { s with
    ...
    rw [stepPos_right (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i)
      (blockRow prog i) (by omega) (by omega)]
  have h2 : run 1 { s with dir := .right, pc := (entryColumn prog i + 1, blockRow prog i) } =
some { s with dir := .right, stack := [n, n], pc := (entryColumn prog i + 2, blockRow prog i) } :=
by
    rw [show run 1 { s with
      dir := .right,
      pc := (entryColumn prog i + 1, blockRow prog i) } = step { s with
  ...
  have h3 : run 1 { s with
    ...
    stack := [n, n],
    pc := (entryColumn prog i + 2, blockRow prog i) } =
some { s with dir := .right, stack := [counterVal c, n, n], pc := (entryColumn prog i + 3, blockRow
prog i) } := by
    rw [show run 1 { s with
      dir := .right,
      stack := [n, n],
  ...
  have h4 : run 1 { s with
    dir := .right,
    stack := [counterVal c, n, n],
    pc := (entryColumn prog i + 3, blockRow prog i) } =
some { s with dir := .right, stack := [n % counterVal c, n], pc := (entryColumn prog i + 4, blockRow
prog i) } := by
    rw [show run 1 { s with
      dir := .right,
      stack := [counterVal c, n, n],
    ...
    rw [stepPos_right (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 3)
      (blockRow prog i) (by omega) (by omega)]
  have h12 : run (1 + 1) s = some { s with
    dir := .right,
    stack := [n, n],
...
prog i) } := by
    exact run_append s { s with
      ...
      (some { s with
        dir := .right,
        stack := [n % counterVal c, n],
        pc := (entryColumn prog i + 4, blockRow prog i) }) (1 + 1 + 1) 1 h123 h4
  rw [show 4 = 1 + 1 + 1 + 1 by omega]
  rw [h1234]
...
theorem jumpBlock_run (prog : CMProgram) (i : ℕ) (hi1 : i + 1 < prog.length)
  ...
  have hc1 : s.grid.get (entryColumn prog i + 1) (blockRow prog i) = '^' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 1 0 (by rw [hjump]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hjump] at hpb
    simpa only [blockBodyAt] using hpb
  have h1 : run 1 s = some { s with
  ...
  have h2 : run 1 { s with dir := .right, pc := (entryColumn prog i + 1, blockRow prog i) } =
    ...
    simp only [hsm, hc1, hdec, stepState]
    rw [stepPos_up (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 1)
      (blockRow prog i) (by omega) (by omega) (by omega)]
  have h12 : run (1 + 1) s = some { s with
    pc := (entryColumn prog i + 1, blockRow prog i - 1),
    dir := .up } := by
    exact run_append s { s with dir := .right, pc := (entryColumn prog i + 1, blockRow prog i) }
...
theorem haltBlock_run (prog : CMProgram) (i : ℕ) (hi1 : i + 1 < prog.length)
  ...
  have hc1 : s.grid.get (entryColumn prog i + 1) (blockRow prog i) = '@' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 1 0 (by rw [hhalt]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hhalt] at hpb
    simpa only [blockBodyAt] using hpb
  have h1 : run 1 s = some { s with
  ...
  have h2 : run 1 { s with
    dir := .right,
    pc := (entryColumn prog i + 1, blockRow prog i) } = none := by
    rw [show run 1 { s with
      dir := .right,
      pc := (entryColumn prog i + 1, blockRow prog i) } = step { s with
        dir := .right,
  ...
  have h12 : run (1 + 1) s = none := by
    exact run_append s { s with
      dir := .right,
      pc := (entryColumn prog i + 1, blockRow prog i) } none 1 1 h1 h2
  rw [show 2 = 1 + 1 by omega]
  rw [h12]
...
theorem deczBlock_decrement (prog : CMProgram) (i : ℕ) (hi1 : i + 1 < prog.length)
  ...
  have hc0 : s.grid.get (entryColumn prog i + 4) (blockRow prog i) = '|' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 4 0 (by rw [hdecz]; norm_num [blockWidth])
      (by exact blockHeight_pos _)
    rw [hdecz] at hpb
    simpa only [blockBodyAt] using hpb
  have hc1 : s.grid.get (entryColumn prog i + 4) (blockRow prog i + 1) = counterDigit c := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 4 1 (by rw [hdecz]; norm_num [blockWidth])
      (by rw [hdecz]; norm_num [blockHeight])
    rw [hdecz] at hpb
    simpa only [blockBodyAt] using hpb
  have hc2 : s.grid.get (entryColumn prog i + 4) (blockRow prog i + 2) = '/' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 4 2 (by rw [hdecz]; norm_num [blockWidth])
      (by rw [hdecz]; norm_num [blockHeight])
    rw [hdecz] at hpb
    simpa only [blockBodyAt] using hpb
  have hc3 : s.grid.get (entryColumn prog i + 4) (blockRow prog i + 3) = '>' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 4 3 (by rw [hdecz]; norm_num [blockWidth])
      (by rw [hdecz]; norm_num [blockHeight])
    rw [hdecz] at hpb
    simpa only [blockBodyAt] using hpb
  have hc4 : s.grid.get (entryColumn prog i + 5) (blockRow prog i + 3) = 'v' := by
    rw [hgrid]
    have hpb := playfield_block_get prog i hi1 5 3 (by rw [hdecz]; norm_num [blockWidth])
      (by rw [hdecz]; norm_num [blockHeight])
    rw [hdecz] at hpb
    simpa only [blockBodyAt] using hpb
  have h1 : run 1 s = some { s with
    ...
    rw [stepPos_down (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 4)
      (blockRow prog i) (by omega) (by omega)]
  have h2 : run 1 { s with
    stack := [n],
    pc := (entryColumn prog i + 4, blockRow prog i + 1),
    dir := .down } =
some { s with stack := [counterVal c, n], pc := (entryColumn prog i + 4, blockRow prog i + 2), dir
:= .down } := by
    rw [show run 1 { s with
      stack := [n],
      pc := (entryColumn prog i + 4, blockRow prog i + 1),
      dir := .down } = step { s with
        stack := [n],
        pc := (entryColumn prog i + 4, blockRow prog i + 1),
        dir := .down } by rfl]
    unfold step
    ...
    rw [stepPos_down (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 4)
      (blockRow prog i + 1) (by omega) (by omega)]
  have h3 : run 1 { s with
    stack := [counterVal c, n],
    pc := (entryColumn prog i + 4, blockRow prog i + 2),
    dir := .down } =
some { s with stack := [n / counterVal c], pc := (entryColumn prog i + 4, blockRow prog i + 3), dir
:= .down } := by
    rw [show run 1 { s with
      stack := [counterVal c, n],
      pc := (entryColumn prog i + 4, blockRow prog i + 2),
    ...
    simp only [hsm, hc2, hdec, stepState, Stack.applyBinary]
    rw [stepPos_down (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 4)
      (blockRow prog i + 2) (by omega) (by omega)]
  have h4 : run 1 { s with
    stack := [n / counterVal c],
    pc := (entryColumn prog i + 4, blockRow prog i + 3),
    dir := .down } =
some { s with stack := [n / counterVal c], pc := (entryColumn prog i + 5, blockRow prog i + 3), dir
:= .right } := by
    rw [show run 1 { s with
      stack := [n / counterVal c],
      pc := (entryColumn prog i + 4, blockRow prog i + 3),
    ...
    have hdec : decodeChar '>' = .right := by unfold decodeChar; rfl
    simp only [hsm, hc3, hdec, stepState]
    rw [stepPos_right (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 4)
      (blockRow prog i + 3) (by omega) (by omega)]
  have h5 : run 1 { s with
    stack := [n / counterVal c],
    pc := (entryColumn prog i + 5, blockRow prog i + 3),
    dir := .right } =
some { s with stack := [n / counterVal c], pc := (entryColumn prog i + 5, blockRow prog i + 4), dir
:= .down } := by
    rw [show run 1 { s with
      stack := [n / counterVal c],
      pc := (entryColumn prog i + 5, blockRow prog i + 3),
      dir := .right } = step { s with
        stack := [n / counterVal c],
        pc := (entryColumn prog i + 5, blockRow prog i + 3),
        dir := .right } by rfl]
    unfold step
    ...
    rw [stepPos_down (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 5)
      (blockRow prog i + 3) (by omega) (by omega)]
  have h12 : run (1 + 1) s = some { s with
    stack := [counterVal c, n],
    pc := (entryColumn prog i + 4, blockRow prog i + 2),
  ...
  have h123 : run (1 + 1 + 1) s = some { s with
    ...
    exact run_append s { s with
      ...
      (some { s with
        stack := [n / counterVal c],
        pc := (entryColumn prog i + 4, blockRow prog i + 3),
        dir := .down }) (1 + 1) 1 h12 h3
  have h1234 : run (1 + 1 + 1 + 1) s = some { s with
    stack := [n / counterVal c],
    pc := (entryColumn prog i + 5, blockRow prog i + 3),
    dir := .right } := by
    exact run_append s { s with
...
theorem deczBlock_jump (prog : CMProgram) (i : ℕ) (hi1 : i + 1 < prog.length)
  ...
  rw [stepPos_up (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 4)
    (blockRow prog i) (by omega) (by omega) (by omega)]
