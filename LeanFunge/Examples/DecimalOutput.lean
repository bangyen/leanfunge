...
A "library" program that reads a decimal integer with `&` and prints its
digits back as characters with `,` — the dual of the `&` parser. It is the
first verified program built from reusable fragments: an extraction loop
divides by 10 (`55+/`) storing the remainders in cells of column 0, and a
print loop fetches them in reverse order, adding 48 to print each digit.
...
* `single_halts`: The `5` run halts after 200 steps.
-/
...
/-- The `5` run halts after 200 steps. -/
theorem single_halts : run 200 singleState = none := by
  decide
