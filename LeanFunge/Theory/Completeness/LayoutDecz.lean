...
theorem deczBlock_prefix (prog : CMProgram) (i : ℕ) (hi1 : i + 1 < prog.length)
  ...
  have h2 : run 1 { s with pc := (entryColumn prog i + 1, blockRow prog i) } =
      some { s with stack := [n, n], pc := (entryColumn prog i + 2, blockRow prog i) } := by
  ...
  have h4 : run 1 { s with stack := [counterVal c, n, n], pc := (entryColumn prog i + 3, blockRow prog i) } =
      some { s with stack := [n % counterVal c, n], pc := (entryColumn prog i + 4, blockRow prog i) } := by
    rw [show run 1 { s with stack := [counterVal c, n, n], pc := (entryColumn prog i + 3, blockRow prog i) } = step { s with stack := [counterVal c, n, n], pc := (entryColumn prog i + 3, blockRow prog i) } by rfl]
    unfold step
    have hdec : decodeChar '%' = .mod := by unfold decodeChar; rfl
    simp only [hsm, hc3, hdec, stepState, Stack.applyBinary]
    rw [hdir, stepPos_right (playfieldWidth prog) (playfieldHeight prog) (entryColumn prog i + 3) (blockRow prog i) (by omega) (by omega)]
  have h12 : run (1 + 1) s = some { s with stack := [n, n], pc := (entryColumn prog i + 2, blockRow prog i) } := by
    exact run_append s { s with pc := (entryColumn prog i + 1, blockRow prog i) }
      (some { s with stack := [n, n], pc := (entryColumn prog i + 2, blockRow prog i) }) 1 1 h1 h2
  have h123 : run (1 + 1 + 1) s = some { s with stack := [counterVal c, n, n], pc := (entryColumn prog i + 3, blockRow prog i) } := by
    exact run_append s { s with stack := [n, n], pc := (entryColumn prog i + 2, blockRow prog i) }
      (some { s with stack := [counterVal c, n, n], pc := (entryColumn prog i + 3, blockRow prog i) }) (1 + 1) 1 h12 h3
  have h1234 : run (1 + 1 + 1 + 1) s =
      some { s with stack := [n % counterVal c, n], pc := (entryColumn prog i + 4, blockRow prog i) } := by
    exact run_append s { s with stack := [counterVal c, n, n], pc := (entryColumn prog i + 3, blockRow prog i) }
