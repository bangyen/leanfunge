...
/-- A jump to `k` in the original run agrees with the clamped jump in the
    normalized run. -/
lemma normalizeAgree_jump (prog : CMProgram) (s t : CMState) (hc1 : s.c1 = t.c1)
    (hc2 : s.c2 = t.c2) (k : ℕ) :
    ...
    · have hslt : s.pc < prog.length := by rw [hpeq]; exact hlt
      ...
      | inc c =>
          ...
          have hinc : CMInstr.incCounter c s = CMInstr.incCounter c t := by
            fin_cases c <;> simp [incCounter, write, CMInstr.read, hc1, hc2, hpeq]
          constructor
          · fin_cases c <;> simp [h1, h2, hinc, encodeState, incCounter, write, CMInstr.read, hc1, hc2]
          · intro s' t' hs' ht'
            rw [h2] at hs'
            rw [h1] at ht'
      ...
      | decz c k =>
          ...
          · have h1 := CMInstr.step_decz_zero prog t c k hget hz
            have h2 := CMInstr.step_decz_zero (normalize prog) s c (min k prog.length) hnorm (by simpa [CMInstr.read, hc1, hc2] using hz)
            constructor
            · simp [h1, h2, encodeState, hc1, hc2]
            · intro s' t' hs' ht'
              rw [h2] at hs'
              rw [h1] at ht'
              injection hs' with hs''
          ...
          · have h1 := CMInstr.step_decz_nonzero prog t c k hget hz
            have h2 := CMInstr.step_decz_nonzero (normalize prog) s c (min k prog.length) hnorm (by simpa [CMInstr.read, hc1, hc2] using hz)
            have hdec : CMInstr.decCounter c s = CMInstr.decCounter c t := by
              fin_cases c <;> simp [decCounter, write, CMInstr.read, hc1, hc2, hpeq]
            have hXY : { CMInstr.decCounter c s with pc := s.pc + 1 }
                = { CMInstr.decCounter c t with pc := t.pc + 1 } := by
              rw [hdec]
              simp [hpeq]
            constructor
            · fin_cases c <;> simp [h1, h2, hdec, encodeState, decCounter, write, CMInstr.read, hc1, hc2]
            · intro s' t' hs' ht'
              rw [h2] at hs'
              rw [h1] at ht'
              injection hs' with hs''
